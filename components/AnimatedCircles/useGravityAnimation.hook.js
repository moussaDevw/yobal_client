...
const useDraw = circles => {
  const nativeCode = useMemo(() => {
    ...
    for (let i = 0; i < circles.length; i++) {
      ...
      nativeCode.push(set(circle.x, add(circle.x, multiply(circle.x, -0.03))));
      nativeCode.push(set(circle.y, add(circle.y, multiply(circle.y, -0.03))));
    }
